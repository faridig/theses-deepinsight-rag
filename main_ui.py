...
async def start():
    ...
    try:
        ...
        if themes:
            sidebar_widgets.append(
                Select(
                    id="Theme", label="Domaine d'étude", values=themes, initial_index=0
                )
            )
        ...
        await cl.Message(
            content="Désolé, une erreur est survenue lors de l'initialisation du service."
        ).send()
        return
    ...
    try:
        ...
        if not svc.ping():
            ...
            await cl.Message(
                content="Le service de recherche est temporairement indisponible. Veuillez réessayer plus tard."
            ).send()
            return
...
async def main(message: cl.Message):
    ...
    try:
        ...
        if current_span and current_span.get_span_context().is_valid:
            ...
            cl.user_session.set(
                f"trace_{response.response[:20]}", format(trace_id, "032x")
            )
    except Exception:
        pass
    ...
    if not source_refs:
        answer_text += "\n\n⚠️ **Note :** Aucun document pertinent n'a été identifié pour étayer cette réponse."
    else:
        answer_text += "\n\n**Sources :** " + ", ".join(
...
async def process_feedback(feedback):
    ...
    try:
        ...
        px_client.log_evaluations(
            SpanEvaluations(dataframe=eval_df, eval_name="Human Feedback")
        )
    except Exception as e:
